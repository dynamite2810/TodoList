...
function Footer({ todos, filter, filters }) {
    ...
    <footer class="footer">
        <span class="todo-count">
            <strong>${todos.filter(filters.active).length <= 1 && todos.filter(filters.active).length + ' item left'}</strong>
            <strong>${todos.filter(filters.active).length > 1 && todos.filter(filters.active).length + ' items left'}</strong>
        </span>
        <ul class="filters">
